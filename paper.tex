...
Because $\Delta$ is small, $\Delta \in \{0, \pm 1, \pm 2\}$,
...
Finally, it is not difficult to see that the computed $\Rv$ 
on line~\eqref{ln:computeR} is, with overwhelming probability,
the image under $\phi$ of some element $\rv' \in \range{2^{2\lambda}}^8$,
