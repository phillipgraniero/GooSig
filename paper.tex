...
set $Z_5 \gets \lfloor (t \cdot \mathit{ch} + r_\text{w2} - r_\text{an}) / \ell \rfloor \in \ZZ$, \AlgSep
...
$h^{\lfloor r_\text{s1} / \ell \rfloor}, h^{\lfloor r_\text{s2} / \ell \rfloor}, h^{\lfloor r_\text{s1w} / \ell \rfloor}, h^{\lfloor r_\text{sa} / \ell \rfloor} \in \GG$ respectively. 
...
possibly plus or minus one.
Hence, because in the real transcript $\lfloor r_\text{w2} / \ell \rfloor$ 
and $\lfloor r_\text{an} / \ell \rfloor$ 
are fresh uniform values in $\range{N/\ell}$, this $Z_5$ is distributed 
...
and 
$Z_4 = g^{\lfloor r_\text{an} / \ell \rfloor} h^{\lfloor r_\text{sa} / \ell \rfloor} / c^{\lfloor r_\text{a} / \ell \rfloor} \in \GG$,
where 
$r_\text{w}, r_\text{w2}, r_\text{a}, r_\text{an}, r_\text{s1w}, r_\text{sa}$
...
$r_\text{w}, r_\text{w2}, r_\text{a}, r_\text{an}, r_\text{s1w}, r_\text{sa} \in \ZZ$
...
where $\phi$ is the homomrphism defined in~\eqref{eq:hom}. 
...
TODO: Use the ZK proof of the interactive protocol. Easy corrolary.


\paragraph{Proof of security.}
TODO: Use the extractor of the interactive protocol. Easy corrolary.
