...
%and write $\hat{\rv} = \big(r_\text{w}, r_\text{w2}, r_\text{s1}, r_\text{a}, r_\text{na}, r_\text{s1w}, r_\text{sa}, r_\text{s2} \big)$
...
% Z_5 \gets \lfloor (t \cdot \mathit{ch} + r_\text{w2} - r_\text{na})/\ell \rfloor 
% \end{array}\right) \in \GG^4 \times \ZZ$, \AlgSep
...
Hence, observe that $c_1, c_2, \Rv, \ell, \mathit{ch}$ are all distributed 
...
It remains to argue that $\Zv_q \in \GG^4 \times \ZZ$ is simulated correctly. 
Let's start with the last component of~$\Zv_q$, namely $Z_5 \in \ZZ$.  
